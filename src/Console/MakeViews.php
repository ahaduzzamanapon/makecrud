...
namespace ahaduzzamanapon\makecrud\Console;

use Illuminate\Console\Command;

use ahaduzzamanapon\makecrud\Services\MakeGlobalService;
use ahaduzzamanapon\makecrud\Services\MakeViewsService;
use Illuminate\Support\Facades\File;
use ahaduzzamanapon\makecrud\Services\PathsAndNamespacesService;
...
class MakeViews extends Command
{
    ...
    {
        $templateViewsDirectory = config('makecrud.views_style_directory');
        $separateStyleAccordingToActions = config('makecrud.separate_style_according_to_actions');
        ...
        else
        {
            ...
            {
                if (!File::exists($this->pathsAndNamespacesService->getmakecrudViewsStubCustom($templateViewsDirectory).DIRECTORY_SEPARATOR.$stub.'.stub'))
                {
                    $this->error('Please create this file: '.$this->pathsAndNamespacesService->getmakecrudViewsStubCustom($templateViewsDirectory).DIRECTORY_SEPARATOR.$stub.'.stub');
                    return;
                }
            }
        }
